	...
	for(int i=1;i<=m;i++){
		for(int j=1;j<=n;j++){
			if(s1[i-1]==s2[j-1]){
				...
				// the three cases defined here are.
				// 1) We are removing character from first string 2) We are removing character from second string 3) We have matched the character by adding or removing.
